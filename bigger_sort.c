...
int		check_before_pivot(t_args *stack_a, int pivot)
{
	...
	i = 0;
	while (stack_a->arg && stack_a->arg != pivot)
	{
		if (stack_a->arg < pivot)
			i++;
		stack_a = stack_a->next;
...
int		is_done_pivot(t_args *stack_a, in pivot)
{
	while (stack_a)
	{
		if (stack_a->arg > pivot)
...
int		partition(t_args **stack_a, t_args **stack_b, int lowest, int highest)
{
	...
	while ((*stack_a)->next)
		(*stack_a) = (*stack_a)->next;
	while ((*stack_a && !is_done_pivot(*stack_a, pivot)))
	{
		arg = (*stack_a)->arg;
		side = which_side(*stack_a, (*stack_a)->arg, pivot);
		len = check_before_pivot(*stack_a, pivot);
		while (++i < len)
			call_exec(stack_a, stack_b, "pb");
		if (arg > pivot && side == 2 || arg == pivot)
		{
			(*stack_a) = (*stack_a)->prev;
		}
		if (arg < pivot && side == 2)
		{
			right_to_left(stack_a, stack_b, arg, pivot);
		}
		if (arg < pivot && side == 1)
			call_exec(stack_a, stack_b, "pb");
	}
